...
const { escapeId } = require('mysql2/promise');
...
const escapeOrderColumn = col => {
  const desc = col[0] === '-' ? ' DESC' : '';

  if (desc) col = col.slice(1);

  return `${escapeId(col)}${desc}`;
}
...
class Dataset {
  ...
  get orderSql() {
    ...
    if (this.sortings.length)
      return ` ORDER BY ${this.sortings.map(escapeOrderColumn).join(', ')}`;
